...
in Section~\ref{sec:appendix-algo} for implementation details.
\end{theorem}
...
\db{F}^{(k)} = \left[\begin{array}{c c c} \db{F}_0^{(k)} & \cdots &
\db{F}_k^{(k)}\end{array}\right]^T, \quad
\cdb{F}^{(k)} = \left[\begin{array}{c c c} \cdb{F}_0^{(k)} & \cdots &
\cdb{F}_k^{(k)}\end{array}\right]^T, \\
\ell_{F}^{(k)} = \left[\begin{array}{c c c} \ell_{F, 0}^{(k)} & \cdots &
\ell_{F, k}^{(k)}\end{array}\right]^T, \quad
\widehat{\ell}_{F}^{(k)} = \left[\begin{array}{c c c}
\widehat{\ell}_{F, 0}^{(k)} & \cdots &
\widehat{\ell}_{F, k}^{(k)}\end{array}\right]^T
\end{gather}
...
\end{equation}
With this, we can express the compensated de Casteljau algorithm as
\begin{align}
b^{(k)} &= U_{k + 1} b^{(k + 1)} \\
...
\(\widehat{r} = (1 - s)(1 + \theta_1)\). Due to this,
...
\Longrightarrow \cdb{F}^{(0)} &=
  \sum_{k = 0}^{n - 1}
  U_1 \cdots U_k \widehat{\ell}_F^{(k)} (1 + \theta_{3k - 1})
  = \sum_{k = 0}^{n - 1}
  \left[\sum_{j = 0}^k \widehat{\ell}_{F, j}^{(k)} (1 + \theta_{3k - 1})
    B_{j, k}(s)\right].
\end{align}
From
\begin{align}
...
\Longrightarrow \widehat{\ell}_{F, j}^{(k)}\left(1 + \theta_{3k - 1}\right)
&= \ell_{F, j}^{(k)} + \left[e_1 \theta_{3k + 5F - 3} + \cdots +
e_{5F - 2} \theta_{3k + 1} +
\rho \cdot \cdb{F - 1}_j^{(k + 1)} \theta_{3k + 1}\right] \\
&\coloneqq \ell_{F, j}^{(k)} + d_{F, j}^{(k)}
\end{align}
we see that
\begin{equation}
\cdb{F}^{(0)} = \db{F}^{(0)} + \sum_{k = 0}^{n - 1}
  \left[\sum_{j = 0}^k d_{F, j}^{(k)}
B_{j, k}(s)\right]
\end{equation}
hence
\begin{equation}
\left|\db{F + 1}^{(0)}\right| =
\left|\db{F}^{(0)} - \cdb{F}^{(0)}\right|
\leq \sum_{j = 0}^k \left|d_{F, j}^{(k)}\right| B_{j, k}(s).
\end{equation}
To put a bound on this, define the following
``tilde'' notation for a sum: if \(A = \sum_j \alpha_j\) then
\(\widetilde{A} = \sum_j \left|\alpha_j\right|\) (similar notation is used
for \(\widetilde{p}(s)\), which is used in the condition number).
With this, we must have
\begin{equation}
\left|d_{F, j}^{(k)}\right| =
\left|e_1 \theta_{3k + 5F - 3} + \cdots +
e_{5F - 2} \theta_{3k + 1} +
\rho \cdot \cdb{F - 1}_j^{(k + 1)} \theta_{3k + 1}\right|
\leq \gamma_{3k + 5F - 3} \cdot \widetilde{\ell}_{F, j}^{(k)}.
\end{equation}
...
\widetilde{\ell}_{1, j}^{(k)} &\leq
  && &&
  \gamma_3 \left(
  (1 - s) \left|\widehat{b}_j\right| +
  s \left|\widehat{b}_{j + 1}\right|\right) \\
\widetilde{\ell}_{2, j}^{(k)} &\leq
  \gamma_5 \cdot
  \widetilde{\ell}_{1, j}^{(k)} &&+
  &&
  \gamma_3 \left(
  (1 - s) \left|\cdb{1}_j\right| +
  s \left|\cdb{1}_{j + 1}\right|\right) \\
\widetilde{\ell}_{3, j}^{(k)} &\leq
  \gamma_{10} \cdot
  \widetilde{\ell}_{2, j}^{(k)} &&+
  &&
  \gamma_3 \left(
  (1 - s) \left|\cdb{2}_j\right| +
  s \left|\cdb{2}_{j + 1}\right|\right) \\
\widetilde{\ell}_{F + 1, j}^{(k)} &\leq
  \gamma_{5F} \cdot
  \widetilde{\ell}_{F, j}^{(k)} &&+
  &&
  \gamma_3 \left(
  (1 - s) \left|\cdb{F}_j\right| +
  s \left|\cdb{F}_{j + 1}\right|\right).
\end{alignat}
To be continued...
\end{proof}
