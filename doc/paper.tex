...
approximations of \(\partial b_j^{(k)}\). The idea behind the compensated
...
\begin{breakablealgorithm}\label{alg:comp-de-casteljau}
...
\begin{figure}\label{fig:error-filtration}
\centering
\begin{tikzpicture}[node distance = 0.2\textwidth]
    \node [box] (bj) {\(b\)};
...
``give up'' and use \(\widehat{\partial b}\) instead of
\(\partial b\) (without keeping around any information about the
...
the rounded
\(\widehat{\partial^{K - 1} b}\) in place of the exact
\(\partial^{K - 1} b\).
...
\begin{breakablealgorithm}\label{alg:k-comp-de-casteljau}
...
\section{Quotes: Selection of Test Cases}
...
\begin{breakablealgorithm}\label{alg:two-prod-fma}
