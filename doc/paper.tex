...
(Section~\ref{sec:appendix-algo}) for implementation details.
\end{theorem}
...
In general, for a sequence \(v_0, \ldots, v_n\) we'll use the notation
\(v = \left[\begin{array}{c c c} v_0 & \cdots &
    v_n\end{array}\right]^T\) to indicate the vector containing all of
the values.
...
\begin{breakablealgorithm}
  ...
  \begin{algorithmic}
    \Function{\(\mathtt{res} = \mathtt{CompDeCasteljauK}\)}{$b, s, K$}
      ...
      \EndFor
      \\
      \State \(\mathtt{res} = \mathtt{SumK}\left(\left[
...
\(6(K - 1)^2\) flops, however the results produced are (empirically)
identical. This makes sense: the whole point of \texttt{SumK}
...
The local errors \(\widetilde{\ell}_{F, j}\) satisfy:
\begin{align}
...
be necessary to get an overall bound on the relative error for
\texttt{CompDeCasteljauK}. So we define
\(L_{F, k} \coloneqq \sum_{j = 0}^k \ell_{F, j}^{(k)} B_{j, k}(s)\) and the
related
...
For \(s \in \left[0, 1\right]\), the Bernstein-type sums defined above
satisfy the following bounds:
\begin{align}
D_{0, n - k} &\leq \left(1 + \gamma_{3k}\right) \cdot \widetilde{p}(s) \\
L_{1, n - k} &\leq \gamma_{3k} \cdot \widetilde{p}(s) \\
D_{F + 1, n - k} &\leq \binom{k + F}{F + 1} \gamma_3 \gamma_{5} \gamma_{10}
  \cdots \gamma_{5F} \cdot \left(1 + \gamma_{3k + 5F + 2}\right)
  \cdot \widetilde{p}(s) \\
L_{F + 2, n - k} &\leq \binom{k + F}{F + 1} \gamma_3 \gamma_{5} \gamma_{10}
  \cdots \gamma_{5F} \cdot \gamma_{3k + 5F + 2} \cdot \widetilde{p}(s).
\label{L-sum-bound}
\end{align}
In particular, this means that
\(D_{F, k} = \mathcal{O}\left(\mach^F\right) \cdot \widetilde{p}(s)\) and
\(L_{F, k} = \mathcal{O}\left(\mach^F\right) \cdot \widetilde{p}(s)\).
\end{lemma}

See the appendix (Section~\ref{sec:appendix-proof-details}) for details.

\begin{lemma}\label{lemma:k-order}
The \(K\)-order error \(\db{K}^{(0)}_0\) satisfies
\begin{equation}
  \left|\db{K - 1}^{(0)}_0 - \cdb{K - 1}^{(0)}_0\right| =
  \left|\db{K}^{(0)}_0\right| \leq \alpha(n) \mach^K
  \widetilde{p}(s).
\end{equation}
\end{lemma}
...
\end{equation}
Since Lemma~\ref{lemma:L-and-D-bounds} (in particular \eqref{L-sum-bound})
tells us that \(\sum_{j = 0}^k \widetilde{\ell}_{F, j}^{(k)} B_{j, k}(s)
= L_{F, k} = \mathcal{O}\left(\mach^F\right)\) ...
\end{proof}
...
\begin{theorem}
  ...
  \begin{equation}
    \frac{\left|p(s) - \mathtt{CompDeCasteljau}(p, s, K)\right|}{
      ...
      \right] +
      \left[\alpha(n) \mach^K + \mathcal{O}\left(
...
\mathcal{O}\left(\mach^F\right) \widetilde{p}(s)\), hence
\begin{align}
\left(\mach + 3 \gamma_{n - 1}^2\right) \left|\sum_{F = 0}^{K - 1}
\cdb{F}_0^{(0)}\right| &\leq
\left[\mach + \mathcal{O}\left(\mach^2\right)\right] \left|p(s)\right| +
\mathcal{O}\left(\mach^{K + 1}\right) \widetilde{p}(s) \\
...
\gamma_{2n - 2}^K \left[\left|p(s)\right| +
  \mathcal{O}\left(\mach\right) \widetilde{p}(s)\right] +
\mathcal{O}\left(\mach^{K + 1}\right) \widetilde{p}(s).
\end{align}
...
&\leq
\left[\mach + \mathcal{O}\left(\mach^2\right)\right] \left|p(s)\right| +
\left[\alpha(n) \mach^K + \mathcal{O}\left(\mach^{K + 1}\right) \right]
\widetilde{p}(s).
\end{align}
...
\section{Appendix: Algorithms}\label{sec:appendix-algo}
...
compensated method for computing a sum of \(n\) numbers. The first is a vector
...
\begin{breakablealgorithm}
  ...
  \begin{algorithmic}
    \Function{\(\mathtt{res} = \mathtt{VecSum}\)}{$p$}
      \State \(n = \texttt{length}(p)\)
      \For{\(j = 2, \ldots, n\)}
        \State \(\left[p_i, p_{i - 1}\right] = \mathtt{TwoSum}\left(
            p_i, p_{i - 1}\right)\)
      \EndFor
    \EndFunction
  \end{algorithmic}
...
\begin{breakablealgorithm}
  ...
  \begin{algorithmic}
    \Function{\(\mathtt{res} = \mathtt{SumK}\)}{$p, K$}
      ...
      \EndFor
      \State \(\mathtt{res} = p_1 \oplus p_2 \oplus \cdots \oplus p_n\)
    \EndFunction
  \end{algorithmic}
\end{breakablealgorithm}
...
\begin{breakablealgorithm}
  ...
  \begin{algorithmic}
    \Function{\(\widehat{\ell} =
      ...
      \EndFor
      \\
      \State \(\widehat{\ell} = \widehat{\ell} \oplus \left(\rho \otimes \delta b\right)\)
    \EndFunction
  \end{algorithmic}
\end{breakablealgorithm}

\section{Appendix: Proof Details}\label{sec:appendix-proof-details}
...
\end{equation}
Using this, along with Lemma~\ref{lemma:ell-tilde}, we have
\begin{align}
...
\gamma_{3k} \cdot \widetilde{p}(s)\).

To see that these inequalities make sense qualitatively, note the
following. It's clear from \eqref{ell-F-k} that if
\(D_{F, k} = \mathcal{O}\left(\mach^F\right) \cdot
\widetilde{p}(s)\) and \(L_{F, k} = \mathcal{O}\left(\mach^F\right) \cdot
\widetilde{p}(s)\) then \(L_{F + 1, k} = \mathcal{O}\left(
\mach^{F + 1}\right) \cdot \widetilde{p}(s)\) and \eqref{d-F-k} shows
that \(D_{F, k} = \mathcal{O}\left(L_{F, k}\right)\). Actually verifying
the quantitative bounds is quite tedious and does not provide any further
insight into the behavior of these errors. For example, the bounds above
are loose bounds and much tighter bounds exist that don't have a compact
formula.
\end{proof}
