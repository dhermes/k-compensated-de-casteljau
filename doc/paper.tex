...
\begin{theorem}[\cite{Jiang2010}]
  ...
  \begin{equation}
    \left|\db{1}_0^{(0)} - \cdb{1}_0^{(0)}\right| \leq
    2 \gamma_{3n + 1} \gamma_{3(n - 1)}
    \sum_{j = 0}^n \left|b_j\right| B_{j, n}(s) =
    2 \gamma_{3n + 1} \gamma_{3(n - 1)} \widetilde{p}(s).
  \end{equation}
\end{theorem}

\noindent Using the bound on the error in \(\db{1}\), the algorithm can
...
\item two errors from the two \(\oplus\) when combining the three
  terms in
  \(\left[\widehat{r} \otimes \cdb{F}_j^{(k + 1)}\right] \oplus
