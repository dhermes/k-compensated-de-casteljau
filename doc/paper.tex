...
For \(s \in \left[0, 1\right]\), the Bernstein-type sum defined above
satisfies the following bound:
\begin{equation}\label{L-sum-bound}
L_{F, n - k} \leq \left[\left(3^F \binom{k}{F - 1} + \bigO{k^{F - 1}}\right)
  \mach^F + \bigO{\mach^{F + 1}}\right] \cdot \widetilde{p}(s).
\end{equation}
In particular, this means that
\(L_{F, k} = \bigO{(3 \mach)^F} \cdot \widetilde{p}(s)\).
\end{lemma}

See the appendix (Section~\ref{sec:appendix-proof-details}) for details on
...
\begin{theorem}
  ...
  \begin{equation}
    \frac{\left|p(s) - \mathtt{CompDeCasteljau}(p, s, K)\right|}{
      ...
      \right] +
      \left[\alpha(n) \mach^K + \bigO{\mach^{K + 1}}\right] \cond{p, s}.
  \end{equation}
\end{theorem}
...
&\leq
\left[\mach + \bigO{\mach^2}\right] \left|p(s)\right| +
\left[\alpha(n) \mach^K + \bigO{\mach^{K + 1}} \right]
\widetilde{p}(s).
\end{align}
...
\end{equation}
which will complete the proof. To see this, first note that for a polynomial
in this basis
